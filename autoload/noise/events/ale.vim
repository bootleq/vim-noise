...
function! noise#events#ale#ALELintPost(sound_id) abort
...
function! noise#events#ale#Register(key, event) abort
  ...
  if a:key == 'all-fixed'
    augroup noise_auto
      execute printf('autocmd User ALELintPost call noise#Handler("noise#events#ale#ALELintPost", %s)', sound_id->shellescape())
    augroup END
  else
